...
const Customdesign = () => {
  const [customdesigns] = useState(Array.from({ length: 8 }, (_, index) => ({
    id: index + 1,
    title: `Custom Design ${index + 1}`,
    image: { src: "https://otcimages.basnop.com/1017922_see-all-products_370.jpeg", alt: `T-shirt ${index + 1} image` },
    rating: 4.5,
    reviews: 444,
    ...
    <div className="bg-[#0b1f113d] p-4 font-mono">
      ...
      <div className='grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center mt-8'>
        {customdesigns.map((item, index) => (
          <Link to={`/product/customdesign/${item.id}`} key={index}>
