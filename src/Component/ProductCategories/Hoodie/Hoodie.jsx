...
const Hoodie = () => {
  const [hoodies] = useState(Array.from({ length: 50 }, (_, index) => ({
    id: index + 1,
    title: `Hoodie t-shirt ${index + 1}`,
    image: { src: "https://wolfattire.com/cdn/shop/files/Black_Oversized_Hoodies_Men_1.jpg?v=1726977326", alt: `T-shirt ${index + 1} image` },
    rating: 4.5,
    reviews: 444,
    ...
    <div className="bg-[#0b1f113d] p-4 font-mono">
      ...
      <div className='grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center mt-8'>
        {hoodies.map((item, index) => (
          <Link to={`/product/hoodie/${item.id}`} key={index}>
