...
const Jacket = () => {
  const [jackets] = useState(
    Array.from({ length: 50 }, (_, index) => ({
      id: index + 1,
      title: `Jacket's t-shirt ${index + 1}`,
      image: {
        src: "https://rukminim2.flixcart.com/image/850/1000/xif0q/jacket/c/u/j/xxl-no-classic-latest-fashion-black-biker-leather-jacket-for-men-original-imagtzbkwz4wjzwb.jpeg?q=20&crop=false",
    ...
    <div className="bg-[#0b1f113d] p-4 font-mono">
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center mt-8">
        {jackets.map((item, index) => (
          <Link to={`/product/jacket/${item.id}`} key={index}>
